	...
	do{
		...
		if(msg_buf){
			...
			if(msg_buf->msgdata){
				...
				msg_buf->msgdata = malloc(msg_buf->msglen);
...
shm_task_mng_t *init_shm_task_mng(int thread_qty, void (*shm_task_handler)(shmem_message_t *, msg_buf_t *))
{
	...
	p_task_msg->task_thread_pool = create_thread_pool(thread_qty, 0x200000);
