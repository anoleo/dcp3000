...
/**
    ...
    @stability Stable
 */
PUBLIC void appweb_assert(bool cond);
#elif ME_MPR_DEBUG_LOGGING
    #undef appweb_assert
    #define appweb_assert(C)	if (C) ; else mprAssert(MPR_LOC, #C)
    #define retvoid_assert(C)	appweb_assert(C)
    #define retval_assert(C, val)	appweb_assert(C)
#else
    #undef appweb_assert
    #define appweb_assert(C)	if (1) ; else {}
    #define retvoid_assert(C)	if (C) ; else return
    #define retval_assert(C, val)	if (C) ; else return (val)
#endif

#define retzero_assert(C)	retval_assert(C, 0)
#define retneg_assert(C)	retval_assert(C, -1)
#define retval_1_assert(C)	retval_assert(C, 1)
