...
static int handshakeMbed(MprSocket *sp)
{
    ...
    appweb_assert(!(mb->ctx.state == MBEDTLS_SSL_HANDSHAKE_OVER));
...
PUBLIC int sniCallback(void *unused, mbedtls_ssl_context *ctx, cuchar *host, size_t len)
{
    ...
    appweb_assert(!mb->configured);

    if (!mb->configured) {
        appweb_assert(ctx->handshake);
        cfg = ssl->config;
