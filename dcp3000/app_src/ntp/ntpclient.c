...
static void set_time(struct ntptime *new, int timezone)
{
	...
	tv_set.tv_sec  = new->coarse - JAN_1970 + (timezone*3600);
...
static void setup_transmit(int usd, char *host, short port, struct ntp_control *ntpc)
{
	...
	printf("ntp setup_transmit : dest ipaddr = %#.8x, port = %#d \n",  sa_dest.sin_addr, sa_dest.sin_port);
...
static void inet_hton(u_int hostip, struct in_addr *sinaddr)
{
    u_char       *ch_sin, *ch_h;

    ch_h = (u_char *) &hostip;
    ch_sin = (u_char *) sinaddr;
...
static void setup_transmit(int usd, u_int host, short port, struct ntp_control *ntpc)
{
	struct sockaddr_in sa_dest;
	memset(&sa_dest,0,sizeof sa_dest);
	sa_dest.sin_family=AF_INET;

	inet_hton(host, &(sa_dest.sin_addr));
...
int sync_ntp(unsigned int host, int timezone)
{
	...
	setup_transmit(usd, host, NTP_PORT, &ntpc);
